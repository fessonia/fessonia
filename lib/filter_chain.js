...
class FilterChain {
  ...
  toString () {
    ...
    this.rootNodes.forEach((rootNode, rootNodeIndex) => {
      ...
      s += this._subchainToString2(rootNode);
    });
    return (s);
  }
  ...
  _subchainToString (node, ins = null, outs = null, distFromSubchainRoot = 0) {
    logger.info(`Processing node ${logger.format(node)}, ${distFromSubchainRoot} away from root`);
    let str = (ins === null ? '' : ins.map((e) => `[${e}] `).join('')) + node.toString();
    logger.info(` --> str = ${str}`);
    const conns = this.connections.get(node);
    logger.info(conns);
    // logger.info(` --> node = ${logger.format(node)}, ${distFromSubchainRoot}: conns = ${logger.format(conns)}`);
    if (conns) { // TODO: fix -- need one level deeper here?
      const connInMap = new Map();
      [...conns.entries()].forEach((e) => {
        return [...e[1].entries()].forEach((e1) => {
          const aPadName = `${e1[0].padPrefix}_${e1[1]}`;
          connInMap.set(e[0], aPadName);
        });
      });
      logger.info(logger.format(connInMap));
      if (outs !== null && outs.length > 0) {
        str += outs.map((e) => `[${e}]`).join('');
      }
      let subchainstr = [...conns.entries()]
        .map((c) => {
          const nextNode = [...c.entries()][0];
          if (node.outputsCount === null || node.outputsCount > 0) {
            const outLabels = [];
            let n = nextNode.outputsCount;
            while (n > 0) { outLabels.push(outs.shift()); n--; }
            // logger.info(` --> ${logger.format(c)} (${i}): ${logger.format(nextNode)},`, outLabels);
    ...
    logger.info(`_subchainToString returning: "${str}"`);
    return (str);
  }
  // eslint-disable-next-line require-jsdoc
  _subchainToString2 (node, ins = [], outs = [], distFromSubchainRoot = 0) {
