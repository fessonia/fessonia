...
class FilterChain {
  ...
  constructor (nodes) {
    this.nodes = nodes
    this.inputs = []
    this.inputNode = nodes[0]
    this.inputPads = nodes[0].inputs
    this.outputNode = nodes[nodes.length - 1]
    this.outputPads = nodes[nodes.length - 1].outputs
  }
  ...
  addInput (input) {
    this.inputs = this.inputs
      .concat(this.validateInputs([input]))
  }
  ...
  streamSpecifier (specifier) {
    if (this.streamSpecifierCallCount === undefined) {
      this.streamSpecifierCallCount = 0
    }
    this.streamSpecifierCallCount++
  ...
  getOutputPad (specifier) {
    if (this.getOutputPadCallCount === undefined) {
      this.getOutputPadCallCount = 0
    }
    this.getOutputPadCallCount++
