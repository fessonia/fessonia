...
class FFmpegCommand {
   ...
   * @param {null|Array<Array<FilterNode|FilterChain|FFmpegInput|number>>} mappings - an array of inputs mapped to this output
   * 
   * @returns {void}
   */
  ...
  /**
   * Get outputs on the FFmpegCommand object
   * @param {Array<Array<FilterNode|FilterChain|FFmpegInput|integer>>} mappings - an array of input mappings
   ...
   * @param {FFmpegInput|FilterNode|FilterChain} fromObject - the object to map from
  ...
  getMappingParameterValue (fromObject, fromStreamIndex) {
    ...
    } else if (fromObject instanceof FilterNode || fromObject instanceof FilterChain) {
      let fromObjectOutputPadName = fromObject.outputPadNames[fromStreamIndex];
      logger.info(`Using filter/filterchain output pad name for fromObject: result = ${fromObjectOutputPadName}`);
