...
class FFmpegCommand extends EventEmitter {
  ...
  getMappingParameterValue (fromObject, fromStreamIndex) {
    if (fromObject instanceof FFmpegInput) {
      ...
      logger.debug(`Searching for fromObject in command inputs: result = ${fromObjInputIndex}`);
      ...
      logger.debug(`Using filter/FilterGraph output pad name for fromObject: result = ${fromObjectOutputPadName}`);
  ...
  toCommand () {
    ...
    for (let map of this._connections.keys()) {
      ...
      logger.debug('toCommand(), handling maps:', map);
      for (let input of this._inputs) {
        // build ordered mapping of input pads
        logger.debug('toCommand(), handling maps, inputs loop:', input);
      }
      for (let output of this._outputs) {
        // build ordered mapping of output pads
        logger.debug('toCommand(), handling maps, outputs loop:', output);
      }
    }
    // Handle outputs
