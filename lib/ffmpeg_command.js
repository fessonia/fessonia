...
class FFmpegCommand extends EventEmitter {
  ...
  connect (from, to) {
    if (!from.hasOwnProperty('from')) {
      ...
      throw new Error(`Invalid argument from: ${util.inspect(from)}. Expected Object value with key 'from' and optional key 'track'.`);
    }
  ...
  getMappingParameterValue (fromObject, fromStreamIndex) {
    if (fromObject instanceof FFmpegInput) {
      ...
      if (config.debug) {
        console.debug(`Searching for fromObject in command inputs: result = ${fromObjInputIndex}`);
      }
      ...
      if (config.debug) {
        console.debug(`Using filter/FilterGraph output pad name for fromObject: result = ${fromObjectOutputPadName}`);
      }
  ...
  toCommand () {
    ...
    for (let map of this._connections.keys()) {
      ...
      if (config.debug) {
        console.debug('toCommand(), handling maps:', map);
      }
      for (let input of this._inputs) {
        // build ordered mapping of input pads
        if (config.debug) {
          console.debug('toCommand(), handling maps, inputs loop:', input);
        }
      }
      for (let output of this._outputs) {
        // build ordered mapping of output pads
        if (config.debug) {
          console.debug('toCommand(), handling maps, outputs loop:', output);
        }
      }
    }
    // Handle outputs
