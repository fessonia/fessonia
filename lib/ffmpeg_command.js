...
class FFmpegCommand extends EventEmitter {
   ...
   * @property {Array<FilterGraph>} _filterGraph - the command's filter graph
   */
   ...
   */
  get filterGraph () { return(this._filterGraph); }
  ...
  toCommand () {
    ...
    this.options.forEach((value, opt) => {
      args.push(`-${opt}`);
      if (value !== null && value !== undefined) { args.push(value.toString()); }
    });
    // Handle inputs
    for (let input of this._inputs) { args = args.concat(input.toCommandArray()); }
    // Handle filterGraph
    const filterGraphString = this._filterGraph.toString();
    if (filterGraphString !== '') {
      args.push('-filter_complex')
      args.push(filterGraphString)
    }
    // Handle outputs
