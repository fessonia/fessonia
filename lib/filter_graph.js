...
class FilterGraph {
   ...
   * @param {string} alias - a user-defined alias for this graph
  ...
  _subgraphToString (node, ins = null, outs = null, distFromSubgraphRoot = 0) {
    logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): node = ${logger.format(node)}, ins = ${logger.format(ins)}, outs = ${logger.format(outs)}`);
    ...
    if (connsFromNode && connsFromNode.size > 0) {
      logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): connsFromNode (truthy) = ${logger.format(connsFromNode)}`);
      ...
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        ...
        logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): In map of connections: currentOutputPad = ${currentOutputPad}, connectedNode = ${logger.format(connectedNode)}, currentMappedInputMap = ${logger.format(currentMappedInputMap)}`);
      ...
      connsFromNode.forEach((padMapping) => {
        logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode... padMapping = ${logger.format(padMapping)}`);
        if (padMapping.toNode) {
          const newIns = segmentOuts
            .filter((outputMapping) =>
              outputMapping.to === padMapping.toNode.padName);
          logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode, constructing inputs for next node... segmentOuts = ${logger.format(padMapping)}, newIns = ${logger.format(newIns)}`);
    ...
    logger.info(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): returning '${str}'`);
    return str;
  }
  ...
  _subgraphLeafNodes (node, distFromSubgraphRoot = 0) {
    ...
    logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): leafNodes = ${logger.format(leafNodes)}`);
    let connsFromNode = this.connections.get(node);
    // logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): connsFromNode = ${logger.format(connsFromNode)}, this.connections = ${logger.format(this.connections)}`);
    if (connsFromNode && connsFromNode.size > 0) {
      // logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): connsFromNode (truthy) = ${logger.format(connsFromNode)}`);
      
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        // For each pad...
        const currentOutputPad = outPadToInEntry[0];
        const currentMappedInputMap = outPadToInEntry[1];
        // logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): currentOutputPad = ${currentOutputPad}, currentMappedInputMap = ${logger.format(currentMappedInputMap)}`);
        let connectedNodes = [...currentMappedInputMap.entries()]
          .map((inNodeInPadPair) => {
            ...
            // logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): connectedNodeRecord = ${logger.format(connectedNodeRecord)}`);
            return connectedNodeRecord;
          }); // Array of {node: inNode, padIndex: inPad, streamType: s, padName: pad_name_str}
        let connectedNode;
        logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): working output pad ${currentOutputPad} mapped to ${logger.format(currentMappedInputMap)}, connectedNodes = ${logger.format(connectedNodes)}`);
        ...
        logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): Completed iteration of connection: ${currentOutputPad} => ${logger.format(connectedNode)}, currentMappedInputMap = ${logger.format(currentMappedInputMap)}`);
        const fromNode = {
          node: node,
      ...
      connsFromNode.forEach((padMapping) => {
        logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... recursive execution for connection ${logger.format(padMapping)}`);
        leafNodes = leafNodes.concat(this._subgraphLeafNodes(padMapping.toNode.node, distFromSubgraphRoot + 1));
        logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... completed recursive execution for connection ${logger.format(padMapping)}: leafNodes = ${logger.format(leafNodes)}`);
        ...
        const leafPos = leafNodes.findIndex((nodeRecord) => {
          ...
          logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in findIndex within forEach of connsFromNode... found? ${found}; nodeRecord = ${logger.format(nodeRecord)}; padMapping.fromNode = ${logger.format(padMapping.fromNode)}`);
          return found;
        });
        logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... leafPos = ${leafPos}`);
        if (leafPos >= 0) {
          logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... identified node record ${logger.format(leafNodes[leafPos])} at index ${leafPos} of leafNodes for removal`);
          const removed = leafNodes.splice(leafPos, 1);
          logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... removed node ${logger.format(removed)} from leafNodes; leafNodes = ${logger.format(leafNodes)}`);
        }
      });
      logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): completed all connections, leafNodes = ${logger.format(leafNodes)}`);
    }
    logger.info(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): returning '${logger.format(leafNodes)}'\n`);
    return leafNodes;
  }
}
