...
class FilterGraph {
   ...
   * @property {Array<Array<FilterNode, integer>, Array<FilterNode, integer>>} connections - connections between nodes in the graph
   */
  constructor (nodes, roots = null, connections = []) {
    ...
    if (config.debug) {
      console.debug(`In constructor: processing connections = ${util.inspect(connections)}`);
    }
  ...
  findOutputNodes () {
    let outNodes = [];
    if (config.debug) {
      console.debug(`findOutputNodes: this.rootNodes = ${util.inspect(this.rootNodes)}`);
    }
  ...
  processInputPads () {
    ...
    this.rootNodes.forEach((n) => {
      if (config.debug) {
        console.debug(`>> Iteration ${++counter} | n = ${n} | this.rootNodes = ${this.rootNodes}`);
      }
      if (n.inputsCount !== null && n.inputsCount > 0) {
        ...
        throw new Error(`Unknown number of input pads for variable input filter ${util.inspect(n)}.`);
      }
    });
    return (pads);
  }
  ...
  processOutputPads () {
    const pads = [];
    if (config.debug) {
      console.debug(`processOutputPads: this.leafNodes = ${util.inspect(this.leafNodes)}`);
    }
    this.leafNodes.forEach((n) => {
      if (config.debug) {
        console.debug(`processOutputPads: forEach n: n.node = ${util.inspect(n.node)}, n.node.outputsCount = ${n.node.outputsCount}, n.node.outputs = ${n.node.outputs}, n.padIndex = ${n.padIndex}, n.streamType = ${n.streamType}`);
      }
  ...
  connect (opts) {
    if (!opts.hasOwnProperty('from') || !opts.hasOwnProperty('to')) {
      throw new Error(`Invalid options argument ${util.inspect(options)} for ${util.inspect(this)} 'connect' method.`);
    }
    ...
    if (opts.from instanceof FFmpegInput) {
      ...
      throw new Error(`I don't know how to connect ${util.inspect(opts.from)} to ${util.inspect(opts.to)}.`);
    }
  }
  ...
  connectNodes (nodeOut, nodeOutIndex, nodeIn, nodeInIndex) {
    ...
    if (nodeOutIndex >= nodeOut.outputsCount && nodeOut.outputType !== 'N') {
      if (config.log_warnings) {
        console.warn(`Unable to connect node ${util.inspect(nodeOut)} at pad index ${nodeOutIndex}: not enough output pads.`);
      }
      validConnection = false;
    }
    if (nodeInIndex >= nodeIn.inputsCount && nodeIn.inputType !== 'N') {
      if (config.log_warnings) {
        console.warn(`Unable to connect node ${util.inspect(nodeIn)} at pad index ${nodeInIndex}: not enough input pads.`);
      }
      validConnection = false;
    }
    if (config.debug) {
      console.debug(`In connectNodes(${util.inspect(nodeOut)}, ${nodeOutIndex}, ${util.inspect(nodeIn)}, ${nodeInIndex}): validConnection = ${validConnection}`);
    }
    if (validConnection) {
      let outConnections;
      ...
      if (this.connections.has(nodeOut)) {
        ...
        if (config.log_warnings && outConnections.has(nodeOutIndex)) {
          console.warn(`Overwriting connection from ${util.inspect(nodeOut)} at pad index ${nodeOutIndex}`);
        }
      ...
      if (config.debug) {
        console.debug(`Connecting node ${util.inspect(nodeOut)} at pad index ${nodeOutIndex} to node ${util.inspect(nodeIn)} at index ${nodeInIndex}: ${util.inspect(outConnections)}`);
      }
  ...
  connectInput (input, inputStreamIndexOrType, nodeIn, nodeInIndex) {
    let validConnection = true;
    if (['v', 'V', 'a', 'A'].includes(inputStreamIndexOrType)) {
      ...
      if (!compatiblePads.includes(nodeOutputPadName)) {
        console.error(`Input stream type ${streamType} not compatible with filter node ${util.inspect(nodeIn)} input pad ${nodeInIndex} stream type ${nodeIn.inputs[nodeInIndex.toLowerCase()]}`);
        validConnection = false;
      }
    }
    if (nodeInIndex >= nodeIn.inputsCount && nodeIn.inputType !== 'N') {
      console.error(`Unable to connect input ${input} to node ${util.inspect(nodeIn)} at pad index ${nodeInIndex}: not enough input pads on FilterNode.`);
      validConnection = false;
    }
    if (validConnection) {
  ...
  processConnections (connections) {
    ...
    if (!validConnectionsDefinition) {
      if (config.log_warnings) {
        console.warn('Parameter connections is invalid. Should be an Array: [ [ [ filterNode1, "0" ], [ filterNode2, "3" ] ], ... ]');
      }
      return;
    }
    connections.forEach((c) => {
      ...
      if (config.debug) {
        console.debug(`Processing ${util.inspect(c)} == ${util.inspect(fromNode)}:${fromIndex} => ${util.inspect(toNode)}:${toIndex}: existingNodes = ${existingNodes}`);
      }
      if (existingNodes) {
        // connection between nodes in this graph
        util.inspect(`Calling connectNodes(${util.inspect(fromNode)}, ${fromIndex}, ${util.inspect(toNode)}, ${toIndex})`);
        this.connectNodes(fromNode, fromIndex, toNode, toIndex);
      } else {
        // connection from outside into this graph
        const FFmpegInput = FilterGraph._loadFFmpegInput();
        if (config.debug) {
          console.debug(`Processing connection from outside ${util.inspect(this)}: fromNode = ${util.inspect(fromNode)}`);
        }
        // TODO: Currently bombing here `TypeError: Right-hand side of 'instanceof' is not callable`
        if (config.debug) {
          console.debug(`ERROR HAPPENS HERE: fromNode = ${util.inspect(fromNode)}, FFmpegInput = ${util.inspect(FFmpegInput)}`);
        }
        if (fromNode instanceof FFmpegInput) {
          if (config.debug) {
            console.debug('fromNode is an FFmpegInput object');
          }
          ...
          if (['v', 'a'].includes(fromIndex)) {
            ...
            if (!(this.inputs.includes(streamType) && this.inputs.includes('n'))) {
              throw new Error(`Mismatched input mapping from input ${fromInput} to filter graph ${util.inspect(this)}: input type ${streamType} doesn't match any filter pad types ${util.inspect(this.inputs)}`);
            }
  ...
  toString () {
    ...
    this.rootNodes.forEach((rootNode, rootNodeIndex) => {
      if (config.debug) {
        console.debug(`toString() > this.rootNodes.forEach: rootNode = ${util.inspect(rootNode)}`);
      }
  ...
  _subgraphToString (node, ins = null, outs = null, distFromSubgraphRoot = 0) {
    if (config.debug) {
      console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): node = ${util.inspect(node)}, ins = ${util.inspect(ins)}, outs = ${util.inspect(outs)}`);
    }
    ...
    if (connsFromNode && connsFromNode.size > 0) {
      if (config.debug) {
        console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): connsFromNode (truthy) = ${util.inspect(connsFromNode)}`);
      }
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        ...
        if (config.debug) {
          console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): In map of connections: currentOutputPad = ${currentOutputPad}, connectedNode = ${util.inspect(connectedNode)}, currentMappedInputMap = ${util.inspect(currentMappedInputMap)}`);
        }
      ...
      connsFromNode.forEach((padMapping) => {
        if (config.debug) {
          console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode... padMapping = ${util.inspect(padMapping)}`);
        }
        if (padMapping.toNode) {
          ...
          if (config.debug) {
            console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode, constructing inputs for next node... segmentOuts = ${util.inspect(padMapping)}, newIns = ${util.inspect(newIns)}`);
          }
    ...
    if (config.debug) {
      console.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): returning '${str}'`);
    }
    return str;
  }
  ...
  _subgraphLeafNodes (node, distFromSubgraphRoot = 0) {
    if (config.debug) {
      console.debug(`\n\nSLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): START node = ${util.inspect(node)}`);
    }
    ...
    if (config.debug) {
      console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): leafNodes = ${util.inspect(leafNodes)}`);
    }
    let connsFromNode = this.connections.get(node);
    if (connsFromNode && connsFromNode.size > 0) {
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        ...
        let connectedNode;
        if (config.debug) {
          console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): working output pad ${currentOutputPad} mapped to ${util.inspect(currentMappedInputMap)}, connectedNodes = ${util.inspect(connectedNodes)}`);
        }
        ...
        if (config.debug) {
          console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): Completed iteration of connection: ${currentOutputPad} => ${util.inspect(connectedNode)}, currentMappedInputMap = ${util.inspect(currentMappedInputMap)}`);
        }
        const fromNode = {
          node: node,
      ...
      connsFromNode.forEach((padMapping) => {
        if (config.debug) {
          console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... recursive execution for connection ${util.inspect(padMapping)}`);
        }
        leafNodes = leafNodes.concat(this._subgraphLeafNodes(padMapping.toNode.node, distFromSubgraphRoot + 1));
        if (config.debug) {
          console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... completed recursive execution for connection ${util.inspect(padMapping)}: leafNodes = ${util.inspect(leafNodes)}`);
        }
        ...
        const leafPos = leafNodes.findIndex((nodeRecord) => {
          ...
          if (config.debug) {
            console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in findIndex within forEach of connsFromNode... found? ${found}; nodeRecord = ${util.inspect(nodeRecord)}; padMapping.fromNode = ${util.inspect(padMapping.fromNode)}`);
          }
          return found;
        });
        if (config.debug) {
          console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... leafPos = ${leafPos}`);
        }
        if (leafPos >= 0) {
          if (config.debug) {
            console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... identified node record ${util.inspect(leafNodes[leafPos])} at index ${leafPos} of leafNodes for removal`);
          }
          const removed = leafNodes.splice(leafPos, 1);
          if (config.debug) {
            console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... removed node ${util.inspect(removed)} from leafNodes; leafNodes = ${util.inspect(leafNodes)}`);
          }
        }
      });
      if (config.debug) {
        console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): completed all connections, leafNodes = ${util.inspect(leafNodes)}`);
      }
    }
    if (config.debug) {
      console.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): returning '${util.inspect(leafNodes)}'\n`);
    }
    return leafNodes;
  }
