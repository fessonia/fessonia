    ...
    if (validConnection) {
      ...
      if (this.connections.has(nodeOut)) {
        ...
        if (outConnections.has(nodeOutIndex)) {
          logger.warn(`Overwriting connection from ${util.inspect(nodeOut)} at pad index ${nodeOutIndex}`);
        }
  ...
  connectInput (input, inputStreamIndexOrType, nodeIn, nodeInIndex) {
    ...
    if (['v', 'V', 'a', 'A'].includes(inputStreamIndexOrType)) {
      ...
      if (!compatiblePads.includes(nodeOutputPadName)) {
        logger.error(`Input stream type ${streamType} not compatible with filter node ${util.inspect(nodeIn)} input pad ${nodeInIndex} stream type ${nodeIn.inputs[nodeInIndex.toLowerCase()]}`);
        validConnection = false;
      }
    }
    if (nodeInIndex >= nodeIn.inputsCount && nodeIn.inputType !== 'N') {
      logger.error(`Unable to connect input ${input} to node ${util.inspect(nodeIn)} at pad index ${nodeInIndex}: not enough input pads on FilterNode.`);
      validConnection = false;
    }
    if (validConnection) {
