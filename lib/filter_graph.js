...
class FilterGraph {
  ...
  constructor (nodes, roots = null, connections = []) {
    ...
    logger.debug(`In constructor: processing connections = ${util.inspect(connections)}`);
  ...
  findOutputNodes () {
    let outNodes = [];
    logger.debug(`findOutputNodes: this.rootNodes = ${util.inspect(this.rootNodes)}`);
  ...
  processInputPads () {
    ...
    this.rootNodes.forEach((n) => {
      logger.debug(`>> Iteration ${++counter} | n = ${n} | this.rootNodes = ${this.rootNodes}`);
  ...
  processOutputPads () {
    const pads = [];
    logger.debug(`processOutputPads: this.leafNodes = ${util.inspect(this.leafNodes)}`);
    this.leafNodes.forEach((n) => {
      logger.debug(`processOutputPads: forEach n: n.node = ${util.inspect(n.node)}, n.node.outputsCount = ${n.node.outputsCount}, n.node.outputs = ${n.node.outputs}, n.padIndex = ${n.padIndex}, n.streamType = ${n.streamType}`);
  ...
  connectNodes (nodeOut, nodeOutIndex, nodeIn, nodeInIndex) {
    ...
    if (nodeOutIndex >= nodeOut.outputsCount && nodeOut.outputType !== 'N') {
      logger.warn(`Unable to connect node ${util.inspect(nodeOut)} at pad index ${nodeOutIndex}: not enough output pads.`);
      validConnection = false;
    }
    if (nodeInIndex >= nodeIn.inputsCount && nodeIn.inputType !== 'N') {
      logger.warn(`Unable to connect node ${util.inspect(nodeIn)} at pad index ${nodeInIndex}: not enough input pads.`);
      validConnection = false;
    }
    logger.debug(`In connectNodes(${util.inspect(nodeOut)}, ${nodeOutIndex}, ${util.inspect(nodeIn)}, ${nodeInIndex}): validConnection = ${validConnection}`);
    if (validConnection) {
      let outConnections;
      ...
      logger.debug(`Connecting node ${util.inspect(nodeOut)} at pad index ${nodeOutIndex} to node ${util.inspect(nodeIn)} at index ${nodeInIndex}: ${util.inspect(outConnections)}`);
  ...
  processConnections (connections) {
    ...
    connections.forEach((c) => {
      ...
      logger.debug(`Processing ${util.inspect(c)} == ${util.inspect(fromNode)}:${fromIndex} => ${util.inspect(toNode)}:${toIndex}: existingNodes = ${existingNodes}`);
      if (existingNodes) {
        ...
        const FFmpegInput = FilterGraph._loadFFmpegInput();
        logger.debug(`Processing connection from outside ${util.inspect(this)}: fromNode = ${util.inspect(fromNode)}`);
        // TODO: Currently bombing here `TypeError: Right-hand side of 'instanceof' is not callable`
        logger.debug(`ERROR HAPPENS HERE: fromNode = ${util.inspect(fromNode)}, FFmpegInput = ${util.inspect(FFmpegInput)}`);
        if (fromNode instanceof FFmpegInput) {
          logger.debug('fromNode is an FFmpegInput object');
  ...
  toString () {
    ...
    this.rootNodes.forEach((rootNode, rootNodeIndex) => {
      logger.debug(`toString() > this.rootNodes.forEach: rootNode = ${util.inspect(rootNode)}`);
  ...
  _subgraphToString (node, ins = null, outs = null, distFromSubgraphRoot = 0) {
    logger.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): node = ${util.inspect(node)}, ins = ${util.inspect(ins)}, outs = ${util.inspect(outs)}`);
    ...
    if (connsFromNode && connsFromNode.size > 0) {
      logger.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): connsFromNode (truthy) = ${util.inspect(connsFromNode)}`);
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        ...
        logger.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): In map of connections: currentOutputPad = ${currentOutputPad}, connectedNode = ${util.inspect(connectedNode)}, currentMappedInputMap = ${util.inspect(currentMappedInputMap)}`);
      ...
      connsFromNode.forEach((padMapping) => {
        logger.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode... padMapping = ${util.inspect(padMapping)}`);
        if (padMapping.toNode) {
          const newIns = segmentOuts
            .filter((outputMapping) =>
              outputMapping.to === padMapping.toNode.padName);
          logger.debug(`SC2S (L${distFromSubgraphRoot}, node ${node.padPrefix}): in mapping of connsFromNode, constructing inputs for next node... segmentOuts = ${util.inspect(padMapping)}, newIns = ${util.inspect(newIns)}`);
  ...
  _subgraphLeafNodes (node, distFromSubgraphRoot = 0) {
    ...
    logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): leafNodes = ${util.inspect(leafNodes)}`);
    let connsFromNode = this.connections.get(node);
    if (connsFromNode && connsFromNode.size > 0) {
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        // For each pad...
        ...
        let connectedNode;
        logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): working output pad ${currentOutputPad} mapped to ${util.inspect(currentMappedInputMap)}, connectedNodes = ${util.inspect(connectedNodes)}`);
        ...
        logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): Completed iteration of connection: ${currentOutputPad} => ${util.inspect(connectedNode)}, currentMappedInputMap = ${util.inspect(currentMappedInputMap)}`);
        const fromNode = {
          node: node,
      ...
      connsFromNode.forEach((padMapping) => {
        logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... recursive execution for connection ${util.inspect(padMapping)}`);
        leafNodes = leafNodes.concat(this._subgraphLeafNodes(padMapping.toNode.node, distFromSubgraphRoot + 1));
        logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... completed recursive execution for connection ${util.inspect(padMapping)}: leafNodes = ${util.inspect(leafNodes)}`);
        ...
        const leafPos = leafNodes.findIndex((nodeRecord) => {
          ...
          logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in findIndex within forEach of connsFromNode... found? ${found}; nodeRecord = ${util.inspect(nodeRecord)}; padMapping.fromNode = ${util.inspect(padMapping.fromNode)}`);
          return found;
        });
        logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... leafPos = ${leafPos}`);
        if (leafPos >= 0) {
          logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... identified node record ${util.inspect(leafNodes[leafPos])} at index ${leafPos} of leafNodes for removal`);
          const removed = leafNodes.splice(leafPos, 1);
          logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): in forEach of connsFromNode... removed node ${util.inspect(removed)} from leafNodes; leafNodes = ${util.inspect(leafNodes)}`);
        }
      });
      logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): completed all connections, leafNodes = ${util.inspect(leafNodes)}`);
    }
    logger.debug(`SLN (L${distFromSubgraphRoot}, node ${node.padPrefix}): returning '${util.inspect(leafNodes)}'\n`);
    return leafNodes;
  }
