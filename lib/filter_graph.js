...
class FilterGraph {
  ...
  findOutputNodes () {
    ...
    this.rootNodes.forEach((rootNode) => {
      outNodes = outNodes.concat(this._subchainLeafNodes(rootNode));
    });
    return outNodes;
  }
   ...
   * @returns {string} the string representation of the sub-chain
   */
  _subchainLeafNodes (node, distFromSubchainRoot = 0) {
    logger.info(`\n\nSLN (L${distFromSubchainRoot}, node ${node.padPrefix}): START node = ${logger.format(node)}`);
    ...
    logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): leafNodes = ${logger.format(leafNodes)}`);
    let connsFromNode = this.connections.get(node);
    // logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): connsFromNode = ${logger.format(connsFromNode)}, this.connections = ${logger.format(this.connections)}`);
    if (connsFromNode && connsFromNode.size > 0) {
      ...
      connsFromNode = [...connsFromNode.entries()].map((outPadToInEntry) => {
        ...
        const currentOutputPad = outPadToInEntry[0];
        const currentMappedInputMap = outPadToInEntry[1];
        // logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): currentOutputPad = ${currentOutputPad}, currentMappedInputMap = ${logger.format(currentMappedInputMap)}`);
        let connectedNodes = [...currentMappedInputMap.entries()]
          .map((inNodeInPadPair) => {
            ...
            // logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): connectedNodeRecord = ${logger.format(connectedNodeRecord)}`);
            return connectedNodeRecord;
          }); // Array of {node: inNode, padIndex: inPad, streamType: s, padName: pad_name_str}
        let connectedNode;
        logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): working output pad ${currentOutputPad} mapped to ${logger.format(currentMappedInputMap)}, connectedNodes = ${logger.format(connectedNodes)}`);
        ...
        logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): Completed iteration of connection: ${currentOutputPad} => ${logger.format(connectedNode)}, currentMappedInputMap = ${logger.format(currentMappedInputMap)}`);
        const fromNode = {
          node: node,
      ...
      connsFromNode.forEach((padMapping) => {
        logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in forEach of connsFromNode... recursive execution for connection ${logger.format(padMapping)}`);
        leafNodes = leafNodes.concat(this._subchainLeafNodes(padMapping.toNode.node, distFromSubchainRoot + 1));
        logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in forEach of connsFromNode... completed recursive execution for connection ${logger.format(padMapping)}: leafNodes = ${logger.format(leafNodes)}`);
        ...
        const leafPos = leafNodes.findIndex((nodeRecord) => {
          ...
          logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in findIndex within forEach of connsFromNode... found? ${found}; nodeRecord = ${logger.format(nodeRecord)}; padMapping.fromNode = ${logger.format(padMapping.fromNode)}`);
          return found;
        });
        logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in forEach of connsFromNode... leafPos = ${leafPos}`);
        if (leafPos >= 0) {
          logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in forEach of connsFromNode... identified node record ${logger.format(leafNodes[leafPos])} at index ${leafPos} of leafNodes for removal`);
          const removed = leafNodes.splice(leafPos, 1);
          logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): in forEach of connsFromNode... removed node ${logger.format(removed)} from leafNodes; leafNodes = ${logger.format(leafNodes)}`);
        }
      });
      logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): completed all connections, leafNodes = ${logger.format(leafNodes)}`);
    }
    logger.info(`SLN (L${distFromSubchainRoot}, node ${node.padPrefix}): returning '${logger.format(leafNodes)}'\n`);
    return leafNodes;
  }
}
