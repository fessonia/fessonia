...
class FFmpegProgressEmitter extends WritableStream {
   ...
   * 
   * @param {Object} options - options for the WriteableStream
   */
  constructor (options) {
    super(options);
  }
  ...
  /**
    ...
    * @param {Function} _callback - callback to call when processing complete
    *
  ...
  _write (chunk, encoding, _callback) {
    data = chunk.toString(encoding = encoding);
