...
class FFmpegProgressEmitter extends Writable {
  ...
  lastProgressChunks (n = 1) {
    if (this.progressData.length === 0) { return this.progressData; }
    if (n === 1) { return this.progressData[this.progressData.length - 1]; }
    const start = Math.max(this.progressData.length - n, 0);
    return this.progressData.slice(start);
  }
  ...
  _write (chunk, encoding, callback) {
    ...
    if (inputMatch != null) {
      this.currentInput = inputMatch[1];
      isProgressLine = true;
    }
    if (timeMatch != null) {
    ...
    if (fpsMatch != null) {
      newFPS = parseFloat(fpsMatch[1]);
      isProgressLine = true;
    }
    if (qMatch != null) {
      newQ = parseFloat(qMatch[1]);
      isProgressLine = true;
    }
    if (sizeMatch != null) {
      newSize = sizeMatch[1];
