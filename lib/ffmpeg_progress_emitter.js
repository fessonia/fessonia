...
class FFmpegProgressEmitter extends Writable {
  ...
  _write (chunk, encoding, callback) {
    const chunkString = (encoding === 'buffer') ? chunk.toString() : chunk.toString(encoding);
    const lastLineComplete = chunkString.endsWith('\n');
    const lines = chunkString.split(/\r\n|\n/);
    lines[0] = this.unprocessedLinePart + lines[0];
    const logLines = [];
    lines.forEach((line) => {
      // NOTE: progress data not separated by line breaks!
