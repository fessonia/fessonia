 ...
 */
class FFmpegProgressEmitter extends Transform {
  /**
   ...
   * 
   * @param {Object} options - options for the Transform stream
   */
  constructor (options) {
    super(options);
  }
  ...
  /**
    ...
    * This method is called internally by Transform stream
    *
  ...
  _transform (chunk, encoding, callback) {
    const data = chunk.toString(encoding = encoding);
    ...
    if (frameMatch != null) {
      newFrame = frameMatch[1];
      progressType = 'video';
    } else {
      progressType = 'audio';
    }
    if (fpsMatch != null) { newFPS = fpsMatch[1]; }
    if (qMatch != null) { newQ = qMatch[1]; }
