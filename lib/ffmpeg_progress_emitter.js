...
class FFmpegProgressEmitter extends Writable {
  ...
  _nextProcessablePiece (s) {
    ...
    let nextPieceMeta;
    if (isProgress) {
      // test for parseability, fail out of progress mode if not
      const returnIndex = crIsNext ? crIndex : nlIndex,
        progressParseable = /=/.test(s.substring(0, returnIndex));
      nextPieceMeta = {
        index: returnIndex,
        type: progressParseable ? 'progress' : 'log',
        continueProgressMode: crIsNext && progressParseable
      };
    } else if (isLog) {
      nextPieceMeta = {
        index: nlIndex,
