...
class FFmpegInput {
  ...
  constructor (url, options = new Map()) {
    ...
    if (config.debug) {
      console.debug(`Pre-validation: ${util.inspect(options)}`);
    }
    this.options = FFmpegInput.validateOptions(options);
    if (config.debug) {
      console.debug(`Post-validation: ${util.inspect(this.options)}`);
    }
  ...
  get inputLabel () {
    if (this._inputLabel === null) {
      if (config.log_warnings) {
        console.warn(`Attempt to retrieve inputLabel before one was set on ${util.inspect(this)}`);
      }
      return undefined;
    }
  ...
  set inputLabel (newLabel) {
    ...
    if (config.debug) {
      console.debug(`Setting inputLabel: newLabel = ${util.inspect(newLabel)}, isString = ${util.inspect(isString)}, hasToString = ${util.inspect(hasToString)}, this._inputLabel = ${util.inspect(this._inputLabel)}`);
    }
    if (isString || hasToString) {
      if (config.log_warnings && this._inputLabel !== null) {
        console.warn(`Resetting existing inputLabel value ${this._inputLabel} on ${util.inspect(this)} to new inputLabel value ${newLabel}.`);
      }
      if (isString) {
      ...
      if (hasToString) {
        if (config.log_warnings) {
          console.warn(`Stringifying new inputLabel value for ${util.inspect(this)}`);
        }
  ...
  markTrackMapped (track) {
    if (typeof track !== 'number') {
      throw new Error(`Expected numeric value for track parameter, but got ${typeof track}: ${util.inspect(track)}`);
    }
    if (this.tracks[track].mapped) {
      throw new Error(`Requested track on input ${util.inspect(this)} already mapped.`);
    }
  ...
  nextAvailableOutputTrack (options = null) {
    if (options !== null && options.hasOwnProperty('streamType')) {
      ...
      throw new Error(`No available track of type '${options.streamType}' found on ${util.inspect(this)}.`);
    }
    this.tracks.forEach((track, index) => {
      if (!track.mapped) return index;
    });
    throw new Error(`No available track found on ${util.inspect(this)}.`);
  }
  ...
  static validateUrl (url) {
    ...
    if (typeof url !== 'string') {
      ...
      if (url instanceof FilterNode || url instanceof FilterGraph) {
        ...
        if (config.debug) {
          console.debug(`Received filter object of type ${result.filterType} as input: converting object ${util.inspect(url)} to string representation: "${result.url}"`);
        }
      } else {
  ...
  static validateOptions (options) {
    ...
    if (config.debug) {
      console.debug(`Validating options: ${util.inspect(options)}`);
    }
    let opts = options;
    if (!(opts instanceof Map)) {
      if (config.debug) {
        console.debug('Validation: opts is not a Map');
      }
      opts = new Map(Object.entries(options));
    }
    if (config.debug) {
      console.debug(`Mid-validation: ${util.inspect(opts)}`);
    }
    ...
    if (config.debug) {
      console.debug(`Completed validating options: ${JSON.stringify(optObjects)}`);
    }
    return (optObjects);
  }
