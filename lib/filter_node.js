...
class FilterNode {
  ...
  getOutputPadIndex (specifier) {
    ...
    if (['v', 'a', 's', 'd', 't'].includes(specifier.toLowerCase())) {
      return this.nextAvailableOutputPadIndex(specifier)
    }
  ...
  markInputPadMapped (padIndex) {
    if (typeof padIndex !== 'number') {
      throw new Error(`Non-numeric input pad index ${padIndex} specified for FilterNode ${util.inspect(this)}.`);
    }
    if (padIndex < 0 || padIndex >= this.inputsCount) {
      throw new Error(`Invalid input pad index ${padIndex} specified for FilterNode ${util.inspect(this)} with ${this.inputsCount} input pads.`);
    }
  ...
  markOutputPadMapped (padIndex) {
    if (typeof padIndex !== 'number') {
      throw new Error(`Non-numeric output pad index ${padIndex} specified for FilterNode ${util.inspect(this)}.`);
    }
    if (padIndex < 0 || padIndex >= this.outputsCount) {
      throw new Error(`Invalid output pad index ${padIndex} specified for FilterNode ${util.inspect(this)} with ${this.outputsCount} output pads.`);
    }
   ...
   * @param {string|undefined} padType - (optional) input pad type: 'v', 'a', 's', 'd', or 't'
   * @returns {number} input pad index
   * @throws
   */
  nextAvailableInputPadIndex (padType) {
    let input, isMapped;
    for (let index = 0; index < this.inputs.length; index++) {
      input = this.inputs[index]
      if (padType !== undefined && input !== padType.toUpperCase() && input !== 'N') { continue; }
      isMapped = this.inputsMapped[index]
      if (isMapped) { continue; }
      return index;
    }
    throw new Error(`No unmapped input pads ${padType !== undefined ? `of type ${padType} ` : ''} found on FilterNode ${util.inspect(this)}`)
  }

  /**
   * Get the next unmapped output pad on this filter node
   * @param {string|undefined} padType - (optional) output pad type: 'v', 'a', 's', 'd', or 't'
   * @returns {number} output pad index
   * @throws
   */
  nextAvailableOutputPadIndex (padType) {
    let output, isMapped;
    for (let index = 0; index < this.outputs.length; index++) {
      output = this.outputs[index]
      if (padType !== undefined && output !== padType.toUpperCase() && output !== 'N') { continue; }
      isMapped = this.outputsMapped[index]
      if (isMapped) { continue; }
      return index;
    }
    throw new Error(`No unmapped output pads ${padType !== undefined ? `of type ${padType} ` : ''} found on FilterNode ${util.inspect(this)}`)
  }
