...
class FFmpegOption {
  ...
  parseArgString (argString) {
    const arrOfArrays = argString.split(':').map((arg) => {
      ...
      const numk = parseInt(kv[0], 10);
      const numv = parseInt(kv[1], 10);
      return ([isNaN(numk) ? kv[0] : numk, isNaN(numv) ? kv[1] : numv]);
    });
  ...
  validateArguments (args) {
    ...
    if (argsIsString) { return this.parseArgString(args); }
    return (new Map(Object.entries(args).map((kv) => {
      const numk = parseInt(kv[0], 10);
      const numv = parseInt(kv[1], 10);
      return ([isNaN(numk) ? kv[0] : numk, isNaN(numv) ? kv[1] : numv]);
    })));
